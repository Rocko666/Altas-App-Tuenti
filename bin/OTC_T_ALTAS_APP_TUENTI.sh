...
VAL_RUTA_SPARK=`mysql -N  <<<"select valor from params where ENTIDAD = '"$ENTIDAD"' AND parametro = 'VAL_RUTA_SPARK';"`
...
echo "Host SFTP: $VAL_FTP_HOSTNAME" 2>&1 &>> $VAL_LOG
echo "Puerto SFTP: $VAL_FTP_PUERTO" 2>&1 &>> $VAL_LOG
echo "Usuario SFTP: $VAL_FTP_USER" 2>&1 &>> $VAL_LOG
echo "Password SFTP: $VAL_FTP_PASS" 2>&1 &>> $VAL_LOG
echo "Ruta SFTP: $VAL_FTP_RUTA" 2>&1 &>> $VAL_LOG
