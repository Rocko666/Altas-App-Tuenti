...
#!/bin/bash
#########################################################################################################
# NOMBRE: OTC_T_TUENTI_MSISDN_BY_ACCOUNT.sh 	    		     	      							    #
# DESCRIPCION:																							#
#   Shell que extrae la información de la tabla otc_t_tuenti_msisdn_by_account de Postgress a Hive		#
...
VAL_DIA=`date '+%Y%m%d'` 
VAL_HORA=`date '+%H%M%S'` 
VAL_LOG=$VAL_RUTA/log/OTC_T_TUENTI_MSISDN_BY_ACCOUNT_$VAL_DIA$VAL_HORA.log
...
#INICIO DEL PROCESO
echo "==== Inicia extraccion tabla OTC_T_TUENTI_MSISDN_BY_ACCOUNT de Postgress ===="`date '+%Y%m%d%H%M%S'` 2>&1 &>> $VAL_LOG
...
if [ $error_spark -eq 0 ];then
	echo "==== OK - La ejecucion del archivo spark otc_t_tuenti_msisdn_by_account.py es EXITOSO ===="`date '+%H%M%S'` 2>&1 &>> $VAL_LOG
	else
	echo "==== ERROR: - En la ejecucion del archivo spark otc_t_tuenti_msisdn_by_account.py ====" 2>&1 &>> $VAL_LOG
	exit 1
fi
...
echo "==== Finaliza extraccion mes actual tabla OTC_T_TUENTI_MSISDN_BY_ACCOUNT de Postgress ===="`date '+%Y%m%d%H%M%S'` 2>&1 &>> $VAL_LOG